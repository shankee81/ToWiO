...
feature %q{
  ...
  context "listing orders" do
    ...
    context "displaying the list of line items" do
      let!(:o1) { FactoryGirl.create(:order_with_distribution, state: 'complete', completed_at: Time.now ) }
      let!(:o2) { FactoryGirl.create(:order_with_distribution, state: 'complete', completed_at: Time.now ) }
      let!(:o3) { FactoryGirl.create(:order_with_distribution, state: 'address', completed_at: nil ) }
    ...
    context "displaying individual columns" do
      let!(:o1) { FactoryGirl.create(:order_with_distribution, state: 'complete', completed_at: Time.now, bill_address: FactoryGirl.create(:address) ) }
      let!(:o2) { FactoryGirl.create(:order_with_distribution, state: 'complete', completed_at: Time.now, bill_address: nil ) }
      let!(:li1) { FactoryGirl.create(:line_item, order: o1 ) }
  ...
  context "using page controls" do
    ...
    context "using drop down seletors" do
      ...
      context "distributor filter" do
        ...
        let!(:o1) { FactoryGirl.create(:order_with_distribution, state: 'complete', completed_at: Time.now, distributor: d1 ) }
        let!(:o2) { FactoryGirl.create(:order_with_distribution, state: 'complete', completed_at: Time.now, distributor: d2 ) }
        let!(:li1) { FactoryGirl.create(:line_item, order: o1 ) }
        let!(:li2) { FactoryGirl.create(:line_item, order: o2 ) }
      ...
      context "order_cycle filter" do
        ...
        let!(:o1) { FactoryGirl.create(:order_with_distribution, state: 'complete', completed_at: Time.now, order_cycle: oc1 ) }
        let!(:o2) { FactoryGirl.create(:order_with_distribution, state: 'complete', completed_at: Time.now, order_cycle: oc2 ) }
        let!(:li1) { FactoryGirl.create(:line_item, order: o1 ) }
        let!(:li2) { FactoryGirl.create(:line_item, order: o2 ) }
      ...
      context "combination of filters" do
        ...
        let!(:o1) { FactoryGirl.create(:order_with_distribution, state: 'complete', completed_at: Time.now, distributor: d1, order_cycle: oc1 ) }
        let!(:o2) { FactoryGirl.create(:order_with_distribution, state: 'complete', completed_at: Time.now, distributor: d2, order_cycle: oc2 ) }
    ...
    context "using quick search" do
      let!(:o1) { FactoryGirl.create(:order_with_distribution, state: 'complete', completed_at: Time.now ) }
      let!(:o2) { FactoryGirl.create(:order_with_distribution, state: 'complete', completed_at: Time.now ) }
      let!(:o3) { FactoryGirl.create(:order_with_distribution, state: 'complete', completed_at: Time.now ) }
      let!(:li1) { FactoryGirl.create(:line_item, order: o1 ) }
      let!(:li2) { FactoryGirl.create(:line_item, order: o2 ) }
      let!(:li3) { FactoryGirl.create(:line_item, order: o3 ) }
    ...
    context "using date restriction controls" do
      let!(:o1) { FactoryGirl.create(:order_with_distribution, state: 'complete', completed_at: (Date.today - 8).strftime("%F %T") ) }
      let!(:o2) { FactoryGirl.create(:order_with_distribution, state: 'complete', completed_at: Time.now ) }
      let!(:o3) { FactoryGirl.create(:order_with_distribution, state: 'complete', completed_at: (Date.today + 2).strftime("%F %T") ) }
